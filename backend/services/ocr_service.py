...
2. engine_number: extract the handwritten/stamped engine number from the row if visible. Use null if not visible.
   - If the engine cell uses ditto marks (" or ||) or visually repeats inside the same operation block, copy the last visible engine number for that operation block.
   - Do not leave continuation/sub-rows blank when the PDF clearly uses ditto marks for the same engine number.
        ...
        try:
            resp = client.models.generate_content(
                ...
                config=google_types.GenerateContentConfig(
                    http_options=google_types.HttpOptions(timeout=GEMINI_TIMEOUT_MS),
                    temperature=0.0,
